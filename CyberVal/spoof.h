...
static uintptr_t ImGayBaseAddy = crt::GetCurrentImageBase();

template <typename Ret, typename... Args>
static inline auto SpoofThis(Ret(*fn)(Args...), Args... args) -> Ret
{
	static const void* jmprbx = nullptr;
	if (!jmprbx) {
		const auto ntdll = reinterpret_cast<const unsigned char*>(ImGayBaseAddy);
